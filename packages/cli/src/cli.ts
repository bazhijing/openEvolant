...
function startServer(opts: {
  ...
  // --- .ns 自然选择配置：预设（仓库 config/ns）+ 用户（根目录 config/ns）---

  /** 解析仓库内 config/ns 目录（预设） */
  function resolveRepoNsDir(): string | null {
    const candidates = [
      path.join(__dirname, '..', '..', 'config', 'ns'),
      path.join(__dirname, '..', '..', '..', 'config', 'ns'),
    ];
    for (const dir of candidates) {
      if (fs.existsSync(dir) && fs.statSync(dir).isDirectory()) return dir;
  ...
  const userNsDir = path.join(opts.configDir, 'ns');
  ...
  // GET 所有 .ns：预设（仓库 config/ns）+ 用户（根目录 config/ns），每项带 source: 'preset' | 'user'
  app.get('/api/config/ns', (_req: Request, res: Response) => {
    try {
      ensureUserNsDirWithExample(userNsDir);

      const presets: Array<Record<string, unknown>> = [];
      const repoNsDir = resolveRepoNsDir();
      if (repoNsDir) {
  ...
  // POST 创建或更新用户 .ns 文件（仅写入 config/ns/<id>.ns）
  app.post('/api/config/ns', (req: Request, res: Response) => {
    try {
      ...
      const payload = {
        ...
        aggregation:
          body.aggregation && typeof body.aggregation === 'object' ? body.aggregation : EXAMPLE_USER_NS.aggregation,
      };
